export class AppError extends Error {
  ...
  static insufficientStock(message: string, available?: number): AppError {
    const err = new AppError(message, 409, 'INSUFFICIENT_STOCK');
    if (available !== undefined) {
      (err as AppError & { available: number }).available = available;
    }
    return err;
  }
}