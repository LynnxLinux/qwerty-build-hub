    ...
    <nav className="sticky top-0 z-50 glass border-b border-border">
      <div className="container mx-auto flex items-center justify-between h-16 px-4">
        ...
        <div className="flex items-center gap-3">
          {isAuthenticated ? (
            <>
              <Link to="/minha-conta" className="hidden md:block text-sm font-medium text-foreground hover:text-foreground-strong transition-colors">
                Minha Conta
              </Link>
        ...
        {mobileOpen && (
          ...
          >
            <div className="p-4 flex flex-col gap-2">
              ...
              {isAuthenticated ? (
                <>
                  <Link to="/minha-conta" onClick={() => setMobileOpen(false)} className="px-4 py-2 rounded-md text-sm font-medium text-foreground hover:bg-accent transition-colors">Minha Conta</Link>
                  <button onClick={() => { logout(); setMobileOpen(false); }} className="px-4 py-2 rounded-md text-sm font-medium text-left text-foreground hover:bg-accent transition-colors">Sair</button>
                </>
              ) : (
                <Link to="/login" onClick={() => setMobileOpen(false)} className="px-4 py-2 rounded-md text-sm font-medium text-primary">Entrar</Link>
              )}
            </div>
          </motion.div>
        )}
